...
def sales_parser():
    ...
    try:
        command = f"C:/google-cloud-sdk/google-cloud-sdk/bin/gsutil cp -r 'Your sales report from google cloud storage{month}.zip' 'C:/revenue/Apps-Revenue-DataPipeline/'"
        os.system(command)
    except Exception as e:
        print("No sales report for this month")
    finally:
        command = f"C:/google-cloud-sdk/google-cloud-sdk/bin/gsutil cp -r 'Your sales report from google cloud storage{prev_month}.zip' 'C:/revenue/Apps-Revenue-DataPipeline/'"
        os.system(command)


    if os.path.exists(f"C:/revenue/Apps-Revenue-DataPipeline/salesreport_2024{month}.zip"):
        with ZipFile(f'C:/revenue/Apps-Revenue-DataPipeline/salesreport_2024{month}.zip', 'r') as zipObj:
            zipObj.extractall()
    else:
        with ZipFile(f'C:/revenue/Apps-Revenue-DataPipeline/salesreport_2024{prev_month}.zip', 'r') as zipObj:
            zipObj.extractall()
    if os.path.exists(f"C:/revenue/Apps-Revenue-DataPipeline/salesreport_2024{month}.csv"):
        report = pd.read_csv(
            f"C:/revenue/Apps-Revenue-DataPipeline/salesreport_2024{month}.csv")
    else:
        report = pd.read_csv(
            f"C:/revenue/Apps-Revenue-DataPipeline/salesreport_2024{prev_month}.csv")
